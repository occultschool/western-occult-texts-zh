...
本資料集根目錄包含 `manifest.sha256`。解壓後可執行：
...
本資料集遵循 LOCKSS 原則：Lots Of Copies Keep Stuff Safe。請將此資料集保存於多個獨立平台，並在新增鏡像後更新本文件。
