# 西方神秘學典籍中文譯庫  
# Western Occult Texts — Chinese Translation Archive

**[繁體中文](#繁體中文) · [简体中文](#简体中文) · [English](#english)**

---

## 繁體中文

### 簡介

本倉庫是「神秘學講堂」（OccultSchool）發布的**西方神秘學典籍中文譯庫**，收錄西方密傳哲學、神智學、卡巴拉、赫密斯主義、玫瑰十字會、占星學及相關靈性傳統的中文翻譯與評注文章，提供**繁體中文**與**簡體中文**兩種版本。

### 內容

本倉庫分為兩個主目錄：

| 目錄 | 說明 |
|------|------|
| `zh-Hant/` | 繁體中文版本 |
| `zh-Hans/` | 簡體中文版本 |

每個主目錄下包含：
- `books/` — 完整書籍翻譯（共 20 部），涵蓋：
  - 布拉瓦茨基著作（《秘密教義》、《寂靜之聲》等）
  - 弗朗茨·哈特曼《與開悟者同行》
  - 《以諾書》《薄伽梵歌》《啟示錄》注釋
  - 及其他西方密傳經典
- `docs/` — 原創評注文章（共 18 個主題分類，1,000+ 篇），包含：
  - 冥想與實修、占星與星座、魔法與靈視
  - 赫爾墨斯主義、生死奧秘
  - 布拉瓦茨基著作導讀、神秘學十二堂課 等

### 文章格式

所有文章均使用 **UTF-8 編碼的 Markdown（.md）**格式，附帶 YAML 前置元數據（標題、描述、難度標籤等）。

### 授權

本作品以 [Creative Commons 姓名標示-相同方式分享 4.0 國際授權條款](https://creativecommons.org/licenses/by-sa/4.0/deed.zh-hant) 發布。  
您可以自由轉載、改編，但須署名「神秘學講堂 (OccultSchool)」並以相同授權分享。

### 引用

```
神秘學講堂 (OccultSchool). (2026). 西方神秘學典籍中文譯庫 [Western Occult Texts — Chinese Translation Archive].
GitHub. https://github.com/occultschool/western-occult-texts-zh
```

---

## 简体中文

### 简介

本仓库是「神秘学讲堂」（OccultSchool）发布的**西方神秘学典籍中文译库**，收录西方密传哲学、神智学、卡巴拉、赫密斯主义、玫瑰十字会、占星学及相关灵性传统的中文翻译与评注文章，提供**繁体中文**与**简体中文**两种版本。

### 内容

| 目录 | 说明 |
|------|------|
| `zh-Hant/` | 繁体中文版本 |
| `zh-Hans/` | 简体中文版本 |

每个主目录下包含：
- `books/` — 完整书籍翻译（共 20 部）
- `docs/` — 原创评注文章（共 18 个主题分类，1,000+ 篇）

### 授权

本作品以 [知识共享 署名-相同方式共享 4.0 国际](https://creativecommons.org/licenses/by-sa/4.0/deed.zh-hans) 许可协议授权。

---

## English

### About

This repository is the **Western Occult Texts — Chinese Translation Archive** published by **OccultSchool** (神秘學講堂). It contains Chinese translations and original commentary articles on Western esoteric philosophy, Theosophy, Kabbalah, Hermeticism, Rosicrucianism, Astrology, and related spiritual traditions.

All content is provided in both **Traditional Chinese (zh-Hant)** and **Simplified Chinese (zh-Hans)**.

### Repository Structure

```
western-occult-texts-zh/
├── zh-Hant/                  # Traditional Chinese
│   ├── books/                # 20 fully translated books
│   │   ├── 秘密教義第一卷/    # The Secret Doctrine, Vol. I (Blavatsky)
│   │   ├── 與開悟者同行/      # Among the Adepts (Hartmann)
│   │   ├── 以諾書/            # The Book of Enoch
│   │   └── ... (17 more)
│   └── docs/                 # 1,000+ original articles (18 categories)
│       ├── 冥想與實修/        # Meditation & Practice
│       ├── 布拉瓦茨基著作/    # Blavatsky Studies
│       ├── 神秘學的十二堂課/  # 12 Lessons in Occultism
│       └── ... (15 more)
└── zh-Hans/                  # Simplified Chinese (mirror content)
    ├── books/
    └── docs/
```

### Format & Longevity

All files are **UTF-8 encoded Markdown** (`.md`) — one of the most future-proof text formats available, human-readable without any software, and trivially convertible to any other format. YAML front-matter encodes structured metadata per document (title, description, keywords, difficulty level).

### Content Scope (Books)

| Title (Chinese) | Subject |
|-----------------|---------|
| 秘密教義第一卷 | The Secret Doctrine Vol. I — H.P. Blavatsky |
| 《秘密教義》筆記 | Notes on the Secret Doctrine |
| 秘傳教義導論 | Introduction to Esoteric Doctrine |
| 與開悟者同行 | Among the Adepts — Franz Hartmann |
| 寂靜之聲 | The Voice of the Silence — H.P. Blavatsky |
| 以諾書 | The Book of Enoch |
| 《薄伽梵歌》四講 | Four Lectures on the Bhagavad Gita |
| 神聖智慧之鑰 | The Key to Theosophy |
| 道路上的光 | Light on the Path |
| 金黃星 | The Golden Star |
| 白蓮田園詩 | The Idyll of the White Lotus |
| 幻想交響曲 | Phantasms (a Spiritual Novel) |
| 宇宙聖母 | Cosmic Virgin |
| 羽翼法老 | The Winged Pharaoh |
| 魯魯的尋求 | Luru's Quest |
| 歐涅斯的《啟示錄》及注釋 | Oanes' Revelation & Commentary |
| 克里希納的真實福音 | The True Gospel of Krishna |
| 從先驅者到詩人 | From Pioneer to Poet |
| 李文和的教誨 | The Teachings of Li Wenhe |
| 秘密教義闡釋 | Elucidation of the Secret Doctrine |

### License

This work is published under the [Creative Commons Attribution-ShareAlike 4.0 International License (CC BY-SA 4.0)](https://creativecommons.org/licenses/by-sa/4.0/).

You are free to share and adapt this material for any purpose, provided you give appropriate credit to **OccultSchool (神秘學講堂)** and distribute your contributions under the same license.

[![CC BY-SA 4.0](https://licensebuttons.net/l/by-sa/4.0/88x31.png)](https://creativecommons.org/licenses/by-sa/4.0/)

### Citing This Work

If you use or reference these translations in academic or published work, please cite:

```
OccultSchool (神秘學講堂). (2026). Western Occult Texts — Chinese Translation Archive
(Version 1.0) [Data set]. GitHub. https://github.com/occultschool/western-occult-texts-zh
DOI: [see CITATION.cff after Zenodo registration]
```

### Preservation

This repository is archived across multiple long-term preservation platforms. See [PRESERVATION.md](./PRESERVATION.md) for the full list of mirrors and integrity verification instructions.

### Contributing

See [CONTRIBUTING.md](./CONTRIBUTING.md).

---

*"That which has no beginning has no end."*  
*「無始者，亦無終。」*
